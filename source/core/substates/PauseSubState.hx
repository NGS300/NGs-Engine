...
class PauseSubState extends MusicBeatSubState {
	...
	override function create() {
		...
		var bg = new BGGraphic(0, 0, FlxG.width, FlxG.height, FlxColor.BLACK, 0, 0);
		bg.alpha = 0;
		add(bg);
		...
		levelInfo.updateHitbox();
			add(levelInfo);*/
		var songName = new HUDText(20, 15, "", "", 30);
		songName.format();
		add(songName);
		...
		var difficultyName = new HUDText(songName.x, songName.y + 32, "", "", 30);
		difficultyName.format();
		add(difficultyName);
	...
	override function update(elapsed:Float) {
		...
		if (controls.ACCEPT) {
			...
			switch (daSelected) {
				...
				case "Exit to menu": changeState(core.states.FreeplayState);
			}
		}
	}
